...
- Internal planning and handoff records are no longer tracked in the public
  repository. Reproducible benchmark and client-probe evidence remains beside
  its public harness and documentation.

### Fixed

- Unverified project routes report `Memory: Off`, including when project
  identity or its AIOS folder cannot be established.
- Unrecognized action wording now gets an honest clarification with a concrete
...
- Legitimate DotAIOS folders created by versions 2.0.8 through 2.0.11 can
  upgrade their exact official skills without treating shipped predecessor
  bytes as user edits.
- Exact schedules generated by 2.0.11 are recognized during managed repair.
- Doctor reports partially projected native skill targets with their live-link
  count and preserves conflicting user files instead of claiming no links exist.

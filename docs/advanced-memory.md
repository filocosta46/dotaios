...
```bash
npx dotaios@2.0.15 memory audit
npx dotaios@2.0.15 memory audit --write-queue
npx dotaios@2.0.15 memory audit --apply-skills
```
...
```bash
npx dotaios@2.0.15 capture enable claude-code   # save sessions automatically
npx dotaios@2.0.15 capture import paste         # or paste one in from any tool
```
...
```bash
npx dotaios@2.0.15 schedule install --dry-run   # shows what it would do, changes nothing
```

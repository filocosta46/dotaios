...
```bash
npx dotaios@2.0.15 project add /path/to/project
npx dotaios@2.0.15 project add /path/to/project --operation-id <displayed-id> --plan-fingerprint <displayed-fingerprint> --apply
npx dotaios@2.0.15 attach /path/to/project
```
...
```bash
npx dotaios@2.0.15 migrate
npx dotaios@2.0.15 project restore <slug-or-id>
```
...
```bash
npx dotaios@2.0.15 project source connect acme-campaign /path/to/assets \
...
```bash
npx dotaios@2.0.15 init --vault-path ~/my-vault
```

Check setup health:

```bash
npx dotaios@2.0.15 status
```

Keep your context fresh:

```bash
npx dotaios@2.0.15 interview --review
npx dotaios@2.0.15 context
npx dotaios@2.0.15 context work --edit
npx dotaios@2.0.15 context --refresh --review
```
...
```bash
npx dotaios@2.0.15 ingest ./notes.md
npx dotaios@2.0.15 ingest https://example.com/article
```

Route by purpose with `--to` to put things in the right place from the start:

```bash
npx dotaios@2.0.15 ingest research.pdf --to wiki --name ai-research           # lasting reference
npx dotaios@2.0.15 ingest brief.pdf --to company --name acme --apply          # org record
npx dotaios@2.0.15 ingest call-notes.txt --to signal                          # working note
```
...
```bash
npx dotaios@2.0.15 capture enable claude-code     # auto-save every Claude Code session
npx dotaios@2.0.15 capture import paste           # paste any conversation manually
npx dotaios@2.0.15 capture import claude-code     # backfill past sessions (last 30 days)
npx dotaios@2.0.15 capture list                   # browse saved conversations
npx dotaios@2.0.15 capture list --agent claude-code --since 7d
npx dotaios@2.0.15 search "any topic"             # search includes saved sessions
```
...
```bash
npx dotaios@2.0.15 brief
```

Search your local AIOS files:

```bash
npx dotaios@2.0.15 search "daily planning" --scope skills
```

Validate and install a plugin from a reviewed local folder:

```bash
npx dotaios@2.0.15 install ./my-plugin --dry-run
npx dotaios@2.0.15 install ./my-plugin
```
...
```bash
npx dotaios@2.0.15 schedule list
```

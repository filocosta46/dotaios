...
- R17. One mutable private working receipt records the merge SHA, candidate SRI, npm metadata and exact comparison, registry tarball probe, npm publisher and dist-tag, tag SHA, GitHub release URL, commands, timestamps, and any incident disposition under `dotaios-product/reports/receipts/releases/`; only after the final success or failure disposition is recorded is it finalized as the immutable release receipt.
...
- **Approach:** Present exact commands that pin `https://registry.npmjs.org/` for effective-registry inspection, `npm whoami`, publication with `--tag latest`, download, and verification. Require the pre-publish identity to be `filippo-costa`. Authentication stays interactive; no token or OTP enters a command argument. Any pre-publication candidate, identity, or expected-E404 check failure records an abort disposition in the working receipt, then aborts without a tag, GitHub release, deprecation, or hotfix requirement. After publication, require `dist.integrity` to equal the candidate SHA-512 SRI, `dist-tags.latest` to equal 2.0.3, the publisher to match exactly, and the registry tarball to pass the isolated install probe. If a post-publication check fails, do not tag: append the incident to the working receipt, deprecate 2.0.3 with an exact warning, open a 2.0.4 hotfix, and consider unpublish only when npm policy and impact justify it. Only after all checks pass, create an annotated `v2.0.3` tag at the squash SHA, push it, create the GitHub release, verify the pinned README and INSTALL URLs, and wait for the tag-triggered freshness workflow. After the final success, pre-publication abort, or post-publication incident disposition is recorded, finalize the working receipt as immutable.
- **Test scenarios:** npm initially returns the expected E404 for 2.0.3; pre-publish identity is exactly `filippo-costa`; a pre-publication failure records the abort and stops without deprecation or a hotfix; after publication npm reports the expected version, `latest` dist-tag, `gitHead`, integrity, publisher, and scripts; the registry package matches and passes the candidate probe; a post-publication failure branch deprecates rather than tags; the successful tag and release target the same SHA; pinned documentation links resolve; release-freshness passes.
- **Verification:** `npm view dotaios@2.0.3 version dist.integrity dist.tarball gitHead _npmUser.name scripts --json`; `npm view dotaios dist-tags --json`; isolated registry-tarball execution; `git rev-parse v2.0.3^{}`; `gh release view v2.0.3`; pinned documentation HTTP checks; GitHub Actions result; working receipt finalized as immutable after its terminal disposition is recorded.
...
| Release receipt | U3-U4 | mutable private working receipt, finalized as the immutable release receipt after the terminal disposition | Every candidate, registry, tag, release, and incident decision remains auditable after temporary files are removed. |
...
- U3 is done when one fresh merged-main release candidate has a retained SHA-512 SRI, executes successfully in an empty isolated prefix, and begins a mutable private working receipt.
- U4 is done when npm integrity equals the candidate SRI, exact publisher and latest dist-tag match, the registry tarball executes in isolation, npm, the annotated tag, the GitHub release, and the freshness workflow all agree on 2.0.3 and the squash commit, pinned documentation links resolve, and the working receipt is finalized as immutable after the successful disposition is recorded. A pre-publication failure instead records the abort, finalizes the receipt, and stops without deprecation or a hotfix requirement. A failed post-publication verification completes only when the version is deprecated, the incident and 2.0.4 hotfix disposition are recorded, and the working receipt is finalized as immutable.

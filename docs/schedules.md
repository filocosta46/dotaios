...
```bash
npx dotaios@2.0.17 schedule due
```

Run every due schedule:

```bash
npx dotaios@2.0.17 schedule run-due
```

Run one schedule:

```bash
npx dotaios@2.0.17 schedule run daily-brief
```

Check local automation setup:

```bash
npx dotaios@2.0.17 schedule doctor
```

Preview an OS scheduler handoff:

```bash
npx dotaios@2.0.17 schedule install --dry-run --target launchd
npx dotaios@2.0.17 schedule install --dry-run --target cron
npx dotaios@2.0.17 schedule install --dry-run --target task-scheduler
```

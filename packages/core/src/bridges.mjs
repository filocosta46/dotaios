...
export const FIRST_TASK_PROMPT = "Help me with one useful task in an existing work folder. Ask what I want to accomplish. If the folder is not connected, ask only for its location; do not require a description. Explain what you understand, propose exactly one action, and wait for my explicit approval before acting.";
...
export async function bridgeManagedBlock(aiosPath, { skillsFirst = false, skillsCatalog, cli, localCli } = {}) {
  ...
  const lines = [
    ...
    legacyDotaios
      ? `- An attached working directory alone is never project identity. For it or \`Only this project\`, run \`${legacyDotaios} project identify --json\` in the same cwd. Host admission reads only registration metadata and returns receipt + \`registered_project\` id/slug + \`aios_folder\`; use the receipt verbatim unless \`Private chat\` locked it. \`Memory: This project\`: read AGENTS.md, then run \`${legacyDotaios} brief --compact --memory project --project <id>\`. \`Memory: Shared\` (cwd inside the AIOS folder): read AGENTS.md, then run \`${legacyDotaios} brief --compact --memory shared\`. \`Memory: Off\`: keep AIOS closed, request registration or \`${legacyDotaios} activate\`.`
      : "- An attached working directory alone is never project identity. For it or `Only this project`, append [`project`,`identify`,`--json`] below and run in the same cwd. Host admission reads only registration metadata and returns receipt + `registered_project` id/slug + `aios_folder`; use the receipt verbatim unless `Private chat` locked it. `Memory: This project`: read AGENTS.md, then append [`brief`,`--compact`,`--memory`,`project`,`--project`,`<id>`]. `Memory: Shared` (cwd inside the AIOS folder): read AGENTS.md, then append [`brief`,`--compact`,`--memory`,`shared`]. `Memory: Off`: keep AIOS closed, register/re-activate.",
    legacyDotaios
  ...
  if (!legacyDotaios) {
    lines.push(
      ...
      `    ${invocation}`,
      "",
      "Pass `candidate_invocation.executable` as the executable and append command arguments to `candidate_invocation.argv_prefix`, without a shell. Do not substitute a package runner, registry lookup, cache search, or bare command.",
      ...
      "2. If `project identify --json` returned an attached registered project that owns the concrete task, keep the task there; do not discover another route.",
      "3. Otherwise derive the current host's native support and invoke implicit discovery. Support only mechanisms this host loads in a fresh project-root child (`agents-md`=AGENTS.md, `claude-md`=CLAUDE.md, `repository-skill`=repository skill); never infer it from a candidate or ask the customer. With support append [\"resolve\",\"<concrete action>\",\"--supports-conventions\",\"<host-native-support>\"]; with none omit the option pair. Use no `--project` and no `--approval-binding`.",
      "4. Non-candidate: weak or vague—ask a concrete action; ambiguous—ask to narrow; `unsupported_by_host`—use a supported local host or manually attach, path-free, no approval. `refused`—stay path-free, relay bounded recovery/next action, never exact-resolve or add; retry implicit only after repair. `no_match/no_registered_projects`—ask only for folder; project add preview `--json`; purpose is optional. Wait for a fresh direct user turn, repeat exact `--operation-id`, `--plan-fingerprint`, `--apply`, `--json`. After apply use only `registered_project` stable ID to exact-resolve the same concrete action with `--project` and the same `--supports-conventions` pair, no `--approval-binding`; then candidate rules. `no_match/concrete_action_required`—ask one concrete action; do not reconnect. Other no-match—project list; add only if absent.",
      "5. Candidate: show only public slug, path-free registration metadata reason, same exact action, approval boundary, fresh context outcome; never disclose a folder path. Retain `approval_binding` opaque; never show the customer. Wait for a fresh direct customer turn; only unambiguous approval continues. Any other response: no exact call, no folder disclosure, no automatic reprompt. After approval exact-resolve the same exact action with identical native support: [\"resolve\",\"<same exact action>\",\"--project\",\"<candidate stable ID>\",\"--supports-conventions\",\"<same host-native-support>\",\"--approval-binding\",\"<retained opaque binding>\"]. Refusal stays path-free.",
      "6. On exact success start fresh ephemeral customer-hidden native child rooted at returned location; return its bounded outcome to same visible task, no second visible task. Carry only higher-priority host authority and approved proposal; no prior project instructions, project memory, governing skill, working-directory binding, or project-scoped tool state. Native project instructions: not route approval, not product authority. Rely on host hierarchy and sandbox to prevent credential access, software installation, out-of-project writes, external submission, or a different action; the router does not provide semantic enforcement. If native launch fails, report it; offer manual opening only of approved folder; must not claim success.",

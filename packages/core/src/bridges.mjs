...
export async function bridgeManagedBlock(aiosPath, { skillsFirst = false, skillsCatalog, cli, localCli } = {}) {
  ...
  if (!legacyDotaios) {
    lines.push(
      ...
      "4. Non-candidates end without a reprompt: weak or vague—ask for a concrete action; ambiguous—ask the customer to narrow it; `unsupported_by_host`—suggest a supported local host or manually attach a known project, remaining path-free with no approval; no match—run project add as a preview for the existing folder and purpose, explain it, await a fresh direct user turn, then repeat with its displayed `--operation-id`, `--plan-fingerprint`, and `--apply`. Never restart automatically.",

...
export async function bridgeManagedBlock(aiosPath, { skillsFirst = false, skillsCatalog, cli, localCli } = {}) {
  ...
  if (!legacyDotaios) {
    lines.push(
      ...
      "4. Non-candidate: weak or vague—ask for a concrete action; ambiguous—ask to narrow; `unsupported_by_host`—suggest a supported local host or manually attach a known project, path-free, no approval. No match: ask only for the folder; project add preview with `--json`. Purpose is optional. Explain, wait for a fresh direct user turn, then repeat with exact `--operation-id`, `--plan-fingerprint`, `--apply`, and `--json`. After apply, take only receipt `registered_project` stable ID; append [\"resolve\",\"<same concrete action>\",\"--project\",\"<returned stable ID>\"] plus the same support pair. Use no `--approval-binding`: this path-free selection is neither approval nor restart. Apply candidate rules.",
      "5. Candidate: show only public slug, path-free registration metadata reason, same exact action, approval boundary, and fresh context outcome; never disclose a folder path. Retain `approval_binding` opaquely and never show the customer. Wait once for a fresh direct customer turn; only unambiguous approval of the unchanged proposal continues. Any other response ends with no exact call, no folder disclosure, no child, and no automatic reprompt. After approval exact-resolve the same exact action and candidate with identical native support by appending [\"resolve\",\"<same exact action>\",\"--project\",\"<candidate stable ID>\",\"--supports-conventions\",\"<same host-native-support>\",\"--approval-binding\",\"<retained opaque binding>\"]. Refusal ends path-free.",
      "6. On exact success start one fresh ephemeral customer-hidden native child rooted at the returned location; return only its bounded outcome to the same visible task, with no second visible task. Carry only higher-priority host authority and the approved proposal; no prior project instructions, project memory, governing skill, working-directory binding, or project-scoped tool state. Native project instructions are not route approval and not product authority. Rely on the host hierarchy and sandbox to prevent credential access, software installation, out-of-project writes, external submission, or a different action; the router does not provide semantic enforcement. If native launch fails, report it, offer manual opening only of the approved folder, and must not claim success.",

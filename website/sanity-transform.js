...
export function docToI18n(doc) {
  ...
  const skillItems = (doc.explorerSkillItems || []).map((row) => ({
    en: ls(row.label, 'en'),
    it: ls(row.label, 'it'),
  }))

  const askItems = (doc.askItems || []).map((row) => ({
  ...
  const build = (lang) => ({
    ...
    explorer: {
      ...
      skills: {
        ...
        items: skillItems.map((row) => row[lang]),
      },
    },
    ask: {
      title: ls(doc.askTitle, lang),
      desc: ls(doc.askDesc, lang),
      items: askItems.map((row) => row[lang]),
